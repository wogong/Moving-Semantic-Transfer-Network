...
class BatchPreprocessor(object):
    ...
    def class_next_batch(self, num_per_class):
        ...
        for i in xrange(31):
            ids += np.random.choice(
                self.classpaths[i], size=num_per_class,
        ...
        # Read images
        images = np.ndarray(
            [num_per_class * 31, self.output_size[0], self.output_size[1], 3])
        for i in range(len(paths)):
            ...
            if self.multi_scale is None:
                ...
                new_size = np.random.randint(self.multi_scale[0],
                                             self.multi_scale[1], 1)[0]
                ...
                if new_size != self.output_size[0]:
                    if self.istraining:
                        ...
                        img = img[random_offset_x:(
                            random_offset_x +
                            self.output_size[0]), random_offset_y:(
                                random_offset_y + self.output_size[0])]
                    else:
                        y, x, _ = img.shape
                        startx = x // 2 - self.output_size[0] // 2
                        starty = y // 2 - self.output_size[1] // 2
                        img = img[starty:starty + self.output_size[0], startx:
                                  startx + self.output_size[1]]
            # Subtract mean color
    ...
    def next_batch(self, batch_size):
        ...
        # Read images
        images = np.ndarray(
            [batch_size, self.output_size[0], self.output_size[1], 3])
        for i in range(len(paths)):
            img = cv2.imread(paths[i])
            # Flip image at random if flag is selected
            if self.horizontal_flip and np.random.random() < 0.5:
