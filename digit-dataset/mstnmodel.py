...
class LeNetModel(object):
    ...
    def __init__(self, num_classes=1000, is_training=True, image_size=28, dropout_keep_prob=0.5):
        ...
        self.range = None
        self.featurelen = 10
        self.source_moving_centroid = tf.get_variable(
            name='source_moving_centroid',
            shape=[num_classes, self.featurelen],
            initializer=tf.zeros_initializer(),
            trainable=False)
        self.target_moving_centroid = tf.get_variable(
            name='target_moving_centroid',
            shape=[num_classes, self.featurelen],
            initializer=tf.zeros_initializer(),
            trainable=False)
    ...
    def adoptimize(self, learning_rate, train_layers=[]):
        ...
        D_op2 = tf.train.MomentumOptimizer(learning_rate * 2.0, 0.9).minimize(
            self.D_loss + self.Dregloss, var_list=D_biases)
    ...
    def optimize(self, learning_rate, train_layers, global_step, source_centroid, target_centroid):
        ...
        with tf.control_dependencies(update_ops):
            train_op3 = tf.train.MomentumOptimizer(learning_rate * 1.0, 0.9).minimize(
                self.F_loss, var_list=new_weights)
...
def D(x):
    with tf.variable_scope('D'):
        ...
        n = 500
        weights = tf.get_variable(
            'weights', shape=[num_units_in, n], initializer=tf.contrib.layers.xavier_initializer())
        ...
        weights3 = tf.get_variable(
            'weights3', shape=[n, num_units_out], initializer=tf.contrib.layers.xavier_initializer())
...
def fc(x, num_in, num_out, name, relu=True, bn=False, stddev=0.001):
    with tf.variable_scope(name) as scope:
        weights = tf.get_variable(
            'weights', shape=[num_in, num_out], initializer=tf.contrib.layers.xavier_initializer())
