...
class SVHN:
    ...
    def load_dataset(self):
        abspaths = {name: self.path + '/' + path for name, path in self.data_files.items()}
    ...
    def class_next_batch(self, num_per_class):
        ...
        selflabels = []
        for i in xrange(10):
            selfimages = np.concatenate(
                (selfimages,
                 self.images[self.classpaths[i][self.class_pointer[i]:self.class_pointer[i] + num_per_class]]), 0)
